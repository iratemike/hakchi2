...
namespace com.clusterrr.Famicom {
	public static class GameGeniePatcher {
		private static readonly Dictionary<char, string> letterValues = new Dictionary<char, string> {
			{'A', "0000"},
			{'P', "0001"},
			{'Z', "0010"},
			{'L', "0011"},
			...
			if (binaryCode.Length == 24) {
				...
				try {
					value = Convert.ToByte(
						new string(new[] {
							binaryCode[0], binaryCode[5], binaryCode[6], binaryCode[7], binaryCode[20], binaryCode[1], binaryCode[2],
							binaryCode[3]
						}), 2);
					address = Convert.ToInt16(
						new string(new[] {
							binaryCode[13], binaryCode[14], binaryCode[15], binaryCode[16], binaryCode[21], binaryCode[22], binaryCode[23],
							binaryCode[4], binaryCode[9], binaryCode[10], binaryCode[11], binaryCode[12], binaryCode[17], binaryCode[18],
							binaryCode[19]
						}), 2);
				} catch {
				...
				try {
					value = Convert.ToByte(
						new string(new[] {
							binaryCode[0], binaryCode[5], binaryCode[6], binaryCode[7], binaryCode[28], binaryCode[1], binaryCode[2],
							binaryCode[3]
						}), 2);
					address = Convert.ToInt16(
						new string(new[] {
					...
					compare = Convert.ToByte(
						new string(new[] {
							binaryCode[24], binaryCode[29], binaryCode[30], binaryCode[31], binaryCode[20], binaryCode[25], binaryCode[26],
							binaryCode[27]
						}), 2);
				} catch {
					throw new GameGenieFormatException(code);
