	...
	public static class IpsPatcher {
		public static void Patch(byte[] patch, ref byte[] data) {
			...
			while (pos < data.Length) {
				var address = (uint) (patch[pos + 2] | (patch[pos + 1] * 0x100) | (patch[pos] * 0x10000));
				if (pos + 3 >= patch.Length) break;
				var length = (ushort) (patch[pos + 4] | (patch[pos + 3] * 0x100));
				...
				if (length > 0) {
					...
					length = (ushort) (patch[pos + 1] | (patch[pos] * 0x100));
					var b = patch[pos + 2];
					while (length > 0) {
						if (address >= data.Length)
							Array.Resize(ref data, data.Length * 2);
