	...
	public partial class SelectModsForm : Form {
		...
		public SelectModsForm(bool loadInstalledMods, bool allowDropMods, string[] filesToAdd = null) {
			...
			if (loadInstalledMods && MainForm.Clovershell.IsOnline) {
				...
				var installedMods = modsstr.Split(new[] {'\n'}, StringSplitOptions.RemoveEmptyEntries);
			...
			readmeFiles = new[] {"readme.txt", "readme.md", "readme"};
		...
		private void loadReadMe(object obj) {
			try {
				...
				Invoke(new Action<string, string>(showReadMe), selected, text);
			} catch {
			}
		}

		private void showReadMe(string mod, string readme) {
			if (checkedListBoxMods.SelectedItem != null &&
