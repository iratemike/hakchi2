...
namespace com.clusterrr.hakchi_gui {
	public partial class ImageGooglerForm : Form {
		...
		public static string[] GetImageUrls(NesMiniApplication app) {
			...
			var search = @"\""ou\""\:\""(?<url>.+?)\""";
			var matches = Regex.Matches(responseFromServer, search);
		...
		private void SearchThread(object o) {
			try {
		...
		private void DownloadImageThread(object o) {
			try {
		...
		protected void ShowImage(Image image) {
			try {
				if (Disposing) return;
				if (InvokeRequired) {
					Invoke(new Action<Image>(ShowImage), image);
					return;
				}
				var i = imageList.Images.Count;
				const int side = 204;
				var imageRect = new Bitmap(side, side, PixelFormat.Format24bppRgb);
				var gr = Graphics.FromImage(imageRect);
				gr.Clear(Color.White);
				if (image.Height > image.Width)
					gr.DrawImage(image,
						new Rectangle((side - side * image.Width / image.Height) / 2, 0, side * image.Width / image.Height, side),
						new Rectangle(0, 0, image.Width, image.Height), GraphicsUnit.Pixel);
				else
					gr.DrawImage(image,
						new Rectangle(0, (side - side * image.Height / image.Width) / 2, side, side * image.Height / image.Width),
						new Rectangle(0, 0, image.Width, image.Height), GraphicsUnit.Pixel);
				gr.Flush();
