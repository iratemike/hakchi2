namespace com.clusterrr.FelLib {
	public class AWFELVerifyDeviceResponse {
		...
		public AWFELVerifyDeviceResponse(byte[] data) {
			...
			Board = (uint) (data[8] | (data[9] * 0x100) | (data[10] * 0x10000) | (data[11] * 0x1000000));
			FW = (uint) (data[12] | (data[13] * 0x100) | (data[14] * 0x10000) | (data[15] * 0x1000000));
			Mode = (ushort) (data[16] | (data[17] * 0x100));
			DataFlag = data[18];
			DataLength = data[19];
			DataStartAddress = (uint) (data[20] | (data[21] * 0x100) | (data[22] * 0x10000) | (data[23] * 0x1000000));
		}
