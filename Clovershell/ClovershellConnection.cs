...
namespace com.clusterrr.clovershell {
	public class ClovershellConnection : IDisposable {
		...
		private byte[] lastPingResponse;
		private Thread mainThread;
		private readonly List<ExecConnection> pendingExecConnections = new List<ExecConnection>();
		private readonly Queue<ShellConnection> pendingShellConnections = new Queue<ShellConnection>();
		...
		public bool ShellEnabled {
			...
			set {
				...
				if (value) {
					...
					Debug.WriteLine("Listening port {0}", shellPort);
					server.Start();
		...
		private void dropAll() {
			...
			foreach (var pending in pendingShellConnections)
				pending.Dispose();
			pendingShellConnections.Clear();
			for (var i = 0; i < execConnections.Length; i++)
		...
		private void mainThreadLoop() {
			try {
				while (enabled) {
					...
					if (IsOnline) Debug.WriteLine("clovershell disconnected");
					IsOnline = false;
					if (device != null)
						device.Close();
					device = null;
		...
		public void Connect() {
			if (Enabled) return;
			Enabled = true;
			while (Enabled && !IsOnline)
				Thread.Sleep(50);
			if (!IsOnline)
				throw new ClovershellException(
					"no clovershell connection, make sure your NES Mini connected, turned on and clovershell mod installed");
		}
		...
		private void killAll() {
			...
			if (tLen != buff.Length)
				throw new ClovershellException("kill all shell: write error - " + r);
			...
			if (tLen != buff.Length)
				throw new ClovershellException("kill all exec: write error - " + r);
		}
		...
		private void shellListenerThreadLoop(object o) {
		...
		private void execResult(byte arg, byte[] data, int pos, int len) {
			var c = execConnections[arg];
			if (c == null) return;
			c.result = data[pos];
			Debug.WriteLine("{0} # exit code: {1}", c.command, c.result);
			c.finished = true;
		}

		private void execStdinStat(byte arg, byte[] data, int pos, int len) {
			var c = execConnections[arg];
			if (c == null) return;
		...
		public int Execute(string command, Stream stdin = null, Stream stdout = null, Stream stderr = null, int timeout = 0,
			bool throwOnNonZero = false) {
			if (!IsOnline) throw new ClovershellException("NES Mini is offline");
			...
			using (var c = new ExecConnection(this, command, stdin, stdout, stderr)) {
				try {
					...
					if (throwOnNonZero && c.result != 0) {
						...
						throw new ClovershellException(string.Format("shell command \"{0}\" returned exit code {1}{2}", command, c.result,
							errText));
					}
					return c.result;
				} finally {
