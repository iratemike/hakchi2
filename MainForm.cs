...
namespace com.clusterrr.hakchi_gui {
	public partial class MainForm : Form {
		...
		private static readonly NesDefaultGame[] defaultNesGames = {
		...
		private static readonly NesDefaultGame[] defaultFamicomGames = {
		...
		private static readonly NesDefaultGame[] defaultSnesGames = {
		...
		private static readonly NesDefaultGame[] defaultSuperFamicomGames = {
		...
		private void FormInitialize() {
			try {
				SyncConsoleType();
				InternalMods = from m in Directory.GetFiles(Path.Combine(Program.BaseDirectoryInternal, "mods/hmods"))
					select Path.GetFileNameWithoutExtension(m);
				LoadPresets();
				LoadLanguages();
				var version = Assembly.GetExecutingAssembly().GetName().Version;
				Text = string.Format("hakchi2 - v{0}.{1:D2}{2}", version.Major, version.Build,
					       version.Revision < 10
						       ? "rc" + version.Revision
						       : (version.Revision > 20 ? ((char) ('a' + (version.Revision - 20) / 10)).ToString() : ""))
#if DEBUG
				       + " (debug version"
#if VERY_DEBUG
 + ", very verbose mode"
				...
				foreach (var ext in app.Extensions)
					if (!extensions.Contains("*" + ext))
						extensions.Add("*" + ext);
				openFileDialogNes.Filter = Resources.GamesAndApps + "|" + string.Join(";", extensions.ToArray()) + "|" +
				                           Resources.AllFiles + "|*.*";
				...
				MessageBox.Show(this, "Critical error: " + ex.Message + ex.StackTrace, Resources.Error, MessageBoxButtons.OK,
					MessageBoxIcon.Error);
			}
		}

		private void Clovershell_OnConnected() {
			try {
				// Trying to autodetect console type
				var customFirmware =
					Clovershell.ExecuteSimple(
						"[ -d /var/lib/hakchi/firmware/ ] && [ -f /var/lib/hakchi/firmware/*.hsqs ] && echo YES || echo NO");
		...
		public void LoadGames() {
			...
			var selected = ConfigIni.SelectedGames.Split(new[] {';'}, StringSplitOptions.RemoveEmptyEntries);
			...
			foreach (var preset in ConfigIni.Presets.Keys.OrderBy(o => o)) {
				presetsToolStripMenuItem.DropDownItems.Insert(i, new ToolStripMenuItem(preset, null,
					delegate {
						...
						var selected = ConfigIni.SelectedGames.Split(new[] {';'}, StringSplitOptions.RemoveEmptyEntries);
						var hide = ConfigIni.HiddenGames.Split(new[] {';'}, StringSplitOptions.RemoveEmptyEntries);
						for (var j = 1; j < listViewGames.Items.Count; j++)
							listViewGames.Items[j].Checked = selected.Contains((listViewGames.Items[j].Tag as NesMiniApplication).Code);
						for (var j = 0; j < checkedListBoxDefaultGames.Items.Count; j++)
				...
				deletePresetToolStripMenuItem.DropDownItems.Insert(i, new ToolStripMenuItem(preset, null,
					delegate {
						if (MessageBox.Show(this, string.Format(Resources.DeletePreset, preset), Resources.AreYouSure,
		...
		private void LoadLanguages() {
			...
			var found = false;
			foreach (var language in langCodes.Keys.OrderBy(o => o)) {
				...
				if (langCodes[language] == "zh-CHS" || langCodes[language] == "zh-CHT") // chinese is awkward
					country = "cn";
				else if (country.Length > 2) country = country.Substring(country.Length - 2).ToLower();
		...
		private void SetImageForSelectedGame(string fileName) {
		...
		private void buttonBrowseImage_Click(object sender, EventArgs e) {
			openFileDialogImage.Filter = Resources.Images + " (*.bmp;*.png;*.jpg;*.jpeg;*.gif)|*.bmp;*.png;*.jpg;*.jpeg;*.gif|" +
			                             Resources.AllFiles + "|*.*";
		...
		private void RecalculateSelectedGamesThread() {
			try {
		...
		private CountResult RecalculateSelectedGames() {
			CountResult stats;
			stats.Count = 0;
			stats.Size = 0;
			if (!IsHandleCreated)
		...
		private void showStats(CountResult stats) {
			try {
				if (InvokeRequired) {
					Invoke(new Action<CountResult>(showStats), stats);
					return;
				}
				...
				toolStripStatusLabelSize.Text = string.Format("{0:F1}MB / {1:F1}MB", stats.Size / 1024.0 / 1024.0,
					maxGamesSize / 1024.0 / 1024.0);
				...
				toolStripStatusLabelSize.ForeColor =
					toolStripProgressBar.Value < toolStripProgressBar.Maximum ? SystemColors.ControlText : Color.Red;
			} catch {
			}
		}
		...
		private DialogResult RequireKernelDump() {
			...
			return DialogResult.No; // Kernel dump cancelled by user
		}

		private DialogResult RequirePatchedKernel() {
		...
		private bool FlashCustomKernel() {
			var workerForm = new WorkerForm(this);
		...
		private bool MembootOriginalKernel() {
			var workerForm = new WorkerForm(this);
		...
		private bool MembootCustomKernel() {
			var workerForm = new WorkerForm(this);
			workerForm.Text = Resources.Membooting;
			workerForm.Task = WorkerForm.Tasks.Memboot;
			workerForm.Mod = "mod_hakchi";
		...
		private bool UploadGames() {
			...
			foreach (ListViewItem game in listViewGames.CheckedItems)
				if (game.Tag is NesMiniApplication)
					...
					needOriginal = true;
			for (var i = 0; i < checkedListBoxDefaultGames.Items.Count; i++)
		...
		private void AddGames(IEnumerable<string> files) {
			SaveConfig();
		...
		private bool FlashOriginalKernel(bool boot = true) {
			var workerForm = new WorkerForm(this);
		...
		private bool InstallMods(string[] mods) {
			var workerForm = new WorkerForm(this);
		...
		private bool UninstallMods(string[] mods) {
			var workerForm = new WorkerForm(this);
		...
		private void dumpKernelToolStripMenuItem_Click(object sender, EventArgs e) {
			...
			if (MessageBox.Show(Resources.DumpKernelQ, Resources.AreYouSure, MessageBoxButtons.YesNo, MessageBoxIcon.Warning)
			    == DialogResult.Yes)
				if (DoKernelDump())
					MessageBox.Show(Resources.KernelDumped, Resources.Done, MessageBoxButtons.OK, MessageBoxIcon.Information);
		}
		...
		private void toolFlashTheWholeNANDStripMenuItem_Click(object sender, EventArgs e) {
			...
			if (MessageBox.Show("It will brick your console. Do you want to continue? :)", Resources.AreYouSure,
				    MessageBoxButtons.YesNo, MessageBoxIcon.Warning)
			    == DialogResult.Yes)
				DoNandFlash();
		}
		...
		private void uninstallToolStripMenuItem_Click(object sender, EventArgs e) {
			...
			if (MessageBox.Show(Resources.UninstallQ1, Resources.AreYouSure, MessageBoxButtons.YesNo, MessageBoxIcon.Warning)
			    == DialogResult.Yes)
				if (Uninstall())
					if (ConfigIni.CustomFlashed && MessageBox.Show(Resources.UninstallQ2, Resources.AreYouSure,
						    MessageBoxButtons.YesNo, MessageBoxIcon.Warning)
					    == DialogResult.Yes)
						if (FlashOriginalKernel())
							MessageBox.Show(Resources.UninstallFactoryNote, Resources.Done, MessageBoxButtons.OK,
		...
		private void donateToolStripMenuItem_Click(object sender, EventArgs e) {
			Process.Start(
				"https://www.paypal.com/cgi-bin/webscr?cmd=_donations&business=clusterrr%40clusterrr%2ecom&lc=RU&item_name=Cluster&currency_code=USD&bn=PP%2dDonationsBF%3abtn_donateCC_LG%2egif%3aNonHosted");
		}
		...
		public void SyncConsoleType() {
			...
			useXYOnClassicControllerAsAutofireABToolStripMenuItem.Enabled =
				ConfigIni.ConsoleType == ConsoleType.NES || ConfigIni.ConsoleType == ConsoleType.Famicom;
			...
			epilepsyProtectionToolStripMenuItem.Checked =
				ConfigIni.AntiArmetLevel > 0 && epilepsyProtectionToolStripMenuItem.Enabled;
			selectButtonCombinationToolStripMenuItem.Enabled =
				resetUsingCombinationOfButtonsToolStripMenuItem.Checked = ConfigIni.ResetHack;
			enableAutofireToolStripMenuItem.Checked = ConfigIni.AutofireHack;
			useXYOnClassicControllerAsAutofireABToolStripMenuItem.Checked =
				ConfigIni.AutofireXYHack && useXYOnClassicControllerAsAutofireABToolStripMenuItem.Enabled;
			upABStartOnSecondControllerToolStripMenuItem.Checked =
				ConfigIni.FcStart && upABStartOnSecondControllerToolStripMenuItem.Enabled;
		...
		private void searchToolStripMenuItem_Click(object sender, EventArgs e) {
			...
			searchForm.Left = Left + 200;
			searchForm.Top = Top + 300;
			searchForm.Show();
		}
		...
		private void buttonShowGameGenieDatabase_Click(object sender, EventArgs e) {
			...
			var nesGame = selected as NesMiniApplication;
			var lFrm = new GameGenieCodeForm(nesGame);
		...
		private bool GroupTaskWithSelected(WorkerForm.Tasks task) {
			var workerForm = new WorkerForm(this);
			switch (task) {
		...
		private void DeleteSelectedGames() {
			if (MessageBox.Show(this, Resources.DeleteSelectedGamesQ, Resources.AreYouSure, MessageBoxButtons.YesNo,
					...
					compressSelectedGamesToolStripMenuItem.Enabled =
						decompressSelectedGamesToolStripMenuItem.Enabled =
							deleteSelectedGamesToolStripMenuItem.Enabled =
								listViewGames.SelectedItems.Count > 1 || listViewGames.SelectedItems.Count == 1 &&
								listViewGames.SelectedItems[0].Tag is NesMiniApplication;
				contextMenuStrip.Show(sender as Control, e.X, e.Y);
			}
		}

		private void listViewGames_KeyDown(object sender, KeyEventArgs e) {
			if (e.KeyCode == Keys.Delete && (listViewGames.SelectedItems.Count > 1 || listViewGames.SelectedItems.Count == 1 &&
			                                 listViewGames.SelectedItems[0].Tag is NesMiniApplication))
				DeleteSelectedGames();
		}
