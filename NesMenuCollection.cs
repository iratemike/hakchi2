...
namespace com.clusterrr.hakchi_gui {
	public class NesMenuCollection : List<INesMenuElement> {
		...
		private const int FoldersEqualLetters = 3;
		private const int PagesEqualLetters = 3;

		public void Split(SplitStyle style, int maxElements = 35) {
			var originalToRoot = false;
			var originalCount = 0;
			switch (style) {
			...
			if ((style == SplitStyle.Auto && !originalToRoot || style == SplitStyle.FoldersEqual ||
			     style == SplitStyle.PagesEqual) &&
			    Count <= maxElements) return;
			var total = Count - originalCount;
			var partsCount = (int) Math.Ceiling(total / (float) maxElements);
			var perPart = (int) Math.Ceiling(total / (float) partsCount);
			...
			if (!originalToRoot) {
				...
				if (root.Count == 0)
					return;
				RemoveAll(o => !(o is NesDefaultGame));
				Add(new NesMenuFolder {
			...
			if (style == SplitStyle.FoldersEqual) // minimum amount of games/folders on screen without glitches
			{
				...
				foreach (var coll in collections) {
					var fname = alphaNum.Replace(
						coll.Where(o => o is NesMiniApplication || o is NesDefaultGame).First().Name.ToUpper(), "");
					...
					var folder = new NesMenuFolder {
						ChildMenuCollection = coll,
						NameParts = new[] {fname, lname},
						Position = NesMenuFolder.Priority.Right
					};
					coll.Add(new NesMenuFolder {
						Name = Resources.FolderNameBack,
						ImageId = "folder_back",
						Position = NesMenuFolder.Priority.Back,
						ChildMenuCollection = root
					});
					root.Add(folder);
				}
				...
				// Pages, equal
			{
				...
				for (i = 0; i < collections.Count; i++) {
					for (var j = i - 1; j >= 0; j--) {
						var fname = alphaNum.Replace(
							collections[j].Where(o => o is NesMiniApplication || o is NesDefaultGame).First().Name.ToUpper(), "");
						var lname = alphaNum.Replace(
							collections[j].Where(o => o is NesMiniApplication || o is NesDefaultGame).Last().Name.ToUpper(), "");
						var folder = new NesMenuFolder {
					...
					for (var j = i + 1; j < collections.Count; j++) {
						var fname = alphaNum.Replace(
							collections[j].Where(o => o is NesMiniApplication || o is NesDefaultGame).First().Name.ToUpper(), "");
						var lname = alphaNum.Replace(
							collections[j].Where(o => o is NesMiniApplication || o is NesDefaultGame).Last().Name.ToUpper(), "");
				...
				foreach (var letter in letters.Keys)
					if (letters[letter].Count > 0) {
						var folderImageId = "folder_" + letter.ToString().ToLower();
						if (letter < 'A' || letter > 'Z') folderImageId = "folder_number";
						var folder = new NesMenuFolder {
							ChildMenuCollection = letters[letter],
			...
			if (originalToRoot)
				if (style != SplitStyle.PagesEqual)
					root.Add(new NesMenuFolder {
						Name = Resources.FolderNameOriginalGames,
						ImageId = "folder_back",
						Position = NesMenuFolder.Priority.Back,
						ChildMenuCollection = this
					});
				else
		...
		public void Unsplit(List<NesMenuCollection> ignore = null) {
			...
			RemoveAll(o => oldElements.Contains(o));
		}
		...
		private void TrimFolderNames(NesMenuCollection nesMenuCollection) {
			...
			for (var i = 1; i < folders.Length; i++) {
				...
				var l = Math.Min(maxChars - 1, Math.Max(nameA.Length, nameB.Length));
				...
				prevFolder.NameParts = new[] {prevFolder.NameParts[0], nameA};
				currentFolder.NameParts = new[] {nameB, currentFolder.NameParts[1]};
			}
			if (folders.Length > 0) {
				var firstFolder = folders[0] as NesMenuFolder;
				firstFolder.NameParts = new[] {
					firstFolder.NameParts[0].Substring(0, Math.Min(firstFolder.NameParts[0].Length, minChars)),
					firstFolder.NameParts[1]
				};

				var lastFolder = folders[folders.Length - 1] as NesMenuFolder;
				lastFolder.NameParts = new[] {
					lastFolder.NameParts[0], lastFolder.NameParts[1].Substring(0, Math.Min(lastFolder.NameParts[1].Length, minChars))
				};
			}
		}
	}
