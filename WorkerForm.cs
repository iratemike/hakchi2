...
namespace com.clusterrr.hakchi_gui {
	public partial class WorkerForm : Form {
		...
		private const ushort vid = 0x1F3A;
		private const ushort pid = 0xEFE8;
		private const long maxCompressedsRamfsSize = 30 * 1024 * 1024;
		public const long ReservedMemory = 10;
		public static int NandCTotal, NandCUsed, NandCFree, WritedGamesSize, SaveStatesSize;
		private readonly string argumentsFilePath;
		private readonly string baseDirectoryExternal;

		private readonly string baseDirectoryInternal;
		private readonly string fes1Path;
		private readonly string hakchiDirectory;
		private readonly string[] hmodDirectories;
		private readonly string initramfs_cpio;
		private readonly string initramfs_cpioPatched;
		private readonly string kernelDirectory;
		private readonly string kernelPatched;
		private readonly string modsDirectory;
		private readonly string ramdiskPatched;
		private readonly string ramfsDirectory;
		private readonly string tempDirectory;
		private readonly string tempHmodsDirectory;
		private readonly string toolsDirectory;
		private readonly string transferDirectory;
		private readonly string ubootPath;
		...
		//string hiddenPath;
		private readonly Dictionary<MainForm.ConsoleType, string[]> correctKernels =
			new Dictionary<MainForm.ConsoleType, string[]>();

		private readonly Dictionary<MainForm.ConsoleType, string[]> correctKeys =
			new Dictionary<MainForm.ConsoleType, string[]>();
		...
		public Tasks Task;
		private string tempGamesDirectory;
		private Thread thread;


		private readonly bool YesForAllPatches = false;
		...
		public WorkerForm(MainForm parentForm) {
			...
			correctKernels[MainForm.ConsoleType.NES] = new[] {
				"5cfdca351484e7025648abc3b20032ff",
				"07bfb800beba6ef619c29990d14b5158"
			};
			correctKernels[MainForm.ConsoleType.Famicom] = new[] {
				"ac8144c3ea4ab32e017648ee80bdc230" // Famicom Mini
			};
			correctKernels[MainForm.ConsoleType.SNES] = new[] {
			...
			correctKernels[MainForm.ConsoleType.SuperFamicom] =
				new[] {"632e179db63d9bcd42281f776a030c14"}; // Super Famicom Mini (JAP)
			correctKeys[MainForm.ConsoleType.NES] = new[] {"bb8f49e0ae5acc8d5f9b7fa40efbd3e7"};
			correctKeys[MainForm.ConsoleType.SNES] = new[] {"c5dbb6e29ea57046579cfd50b124c9e1"};
		}
		...
		private DialogResult WaitForFelFromThread() {
			if (InvokeRequired)
		...
		private DialogResult WaitForClovershellFromThread() {
			if (InvokeRequired)
		...
		public static DialogResult MessageBoxFromThread(IWin32Window owner, string text, string caption,
			MessageBoxButtons buttons,
			MessageBoxIcon icon, MessageBoxDefaultButton defaultButton, bool tweak) {
			if ((owner as Form).InvokeRequired)
				return (DialogResult) (owner as Form).Invoke(new MessageBoxFromThreadDelegate(MessageBoxFromThread), owner, text,
					caption, buttons, icon, defaultButton, tweak);
		...
		private DialogResult FoldersManagerFromThread(NesMenuCollection collection) {
			if (InvokeRequired)
				return (DialogResult) Invoke(new Func<NesMenuCollection, DialogResult>(FoldersManagerFromThread), collection);
		...
		private DialogResult SelectFileFromThread(string[] files) {
			if (InvokeRequired)
		...
		public void StartThread() {
			...
			try {
				DialogResult = DialogResult.None;
				Debug.WriteLine("Executing task: " + Task);
				switch (Task) {
		...
		private void SetProgress(int value, int max) {
			...
			try {
				if (InvokeRequired) {
					Invoke(new Action<int, int>(SetProgress), value, max);
					if (value == max)
						Thread.Sleep(1000);
					return;
				}
		...
		private void ShowError(Exception ex, bool dontStop = false) {
			if (Disposing) return;
			try {
				if (InvokeRequired) {
					Invoke(new Action<Exception, bool>(ShowError), ex, dontStop);
					return;
				}
		...
		private void ShowMessage(string text, string title) {
			...
			try {
				if (InvokeRequired) {
					Invoke(new Action<string, string>(ShowMessage), text, title);
					return;
				}
				TaskbarProgress.SetState(this, TaskbarProgress.TaskbarStates.Paused);
		...
		public bool DoKernelDump(string dumpPath = null, int maxProgress = 80, int progress = 0) {
			...
			if (matchedKernels.Count() == 0) {
				...
				var kernelDumpTemp = Path.Combine(tempDirectory, "kernel.img");
		...
		public void FlashKernel() {
			var progress = 0;
			var maxProgress = 115 + (string.IsNullOrEmpty(Mod) ? 0 : 110) +
			...
			var r = fel.ReadFlash(Fel.kernel_base_f, (uint) kernel.Length,
				delegate(Fel.CurrentAction action, string command) {
					switch (action) {
						case Fel.CurrentAction.RunningCommand:
							SetStatus(Resources.ExecutingCommand + " " + command);
		...
		public static void GetMemoryStats() {
			var clovershell = MainForm.Clovershell;
			var nandc = clovershell
				.ExecuteSimple("df /dev/nandc | tail -n 1 | awk '{ print $2 \" | \" $3 \" | \" $4 }'", 500, true).Split('|');
			WritedGamesSize =
				int.Parse(clovershell.ExecuteSimple(
					"mkdir -p /var/lib/hakchi/rootfs/usr/share/games/ && du -s /var/lib/hakchi/rootfs/usr/share/games/ | awk '{ print $1 }'",
					1000, true)) * 1024;
			SaveStatesSize =
				int.Parse(clovershell.ExecuteSimple(
					"mkdir -p /var/lib/clover/profiles/0/ && du -s /var/lib/clover/profiles/0/ | awk '{ print $1 }'", 1000, true)) *
				1024;
			NandCTotal = int.Parse(nandc[0]) * 1024;
			NandCUsed = int.Parse(nandc[1]) * 1024;
			NandCFree = int.Parse(nandc[2]) * 1024;
			Debug.WriteLine("NANDC size: {0:F1}MB, used: {1:F1}MB, free: {2:F1}MB", NandCTotal / 1024.0 / 1024.0,
				NandCUsed / 1024.0 / 1024.0, NandCFree / 1024.0 / 1024.0);
			Debug.WriteLine("Used by games: {0:F1}MB", WritedGamesSize / 1024.0 / 1024.0);
			Debug.WriteLine("Used by save-states: {0:F1}MB", SaveStatesSize / 1024.0 / 1024.0);
			Debug.WriteLine("Used by other files (mods, configs, etc.): {0:F1}MB",
				(NandCUsed - WritedGamesSize - SaveStatesSize) / 1024.0 / 1024.0);
			Debug.WriteLine("Available for games: {0:F1}MB", (NandCFree + WritedGamesSize) / 1024.0 / 1024.0);
		}
		...
		public void UploadGames() {
			var gamesPath = NesMiniApplication.GamesCloverPath;
			...
			try {
				...
				var originalGames = new Dictionary<string, string>();
				...
				using (var gamesTar = new TarStream(tempGamesDirectory)) {
					...
					clovershell.ExecuteSimple(
						string.Format("rm -rf {0}{1}/CLV-* {0}{1}/??? {2}/menu", rootFsPath, gamesPath, installPath), 5000, true);
		...
		public static void SyncConfig(Dictionary<string, string> Config, bool reboot = false) {
			...
			clovershell.Execute(
				"temppath=/tmp && source /etc/preinit && script_init && source /tmp/config && source $preinit.d/pffff_config", null,
				null, null, 30000, true);
			config.Dispose();
			if (reboot)
				try {
					clovershell.ExecuteSimple("reboot", 100);
		...
		public void Memboot(int maxProgress = -1, int progress = 0) {
			...
			progress += 5;
			if (maxProgress < 0)
				maxProgress = (int) (kernel.Length / (double) 67000 + waitSeconds * 2 + 12);
			SetProgress(progress, maxProgress);
		...
		private void UnpackRamfs(string kernelPath = null) {
			...
			var tempKernelDump = Path.Combine(tempDirectory, "kernel.img");
		...
		private byte[] CreatePatchedKernel(string kernelPath = null) {
			...
			if (!ExecuteTool("mkbootimg.exe", string.Format(
				"--kernel \"{0}\" --ramdisk \"{1}\" --cmdline \"{2}\" --board \"{3}\" --base \"{4}\" --pagesize \"{5}\" --kernel_offset \"{6}\" --ramdisk_offset \"{7}\" --tags_offset \"{8}\" -o \"{9}\"",
				Path.Combine(kernelDirectory, "kernel.img-zImage"), ramdiskPatched, argCmdline, argBoard, argBase, argPagesize,
				argKerneloff, argRamdiscoff, argTagsoff, kernelPatched)))
		...
		private void AddMenu(NesMenuCollection menuCollection, Dictionary<string, string> originalGames,
			GamesTreeStats stats = null) {
			if (stats == null)
				stats = new GamesTreeStats();
			if (!stats.allMenus.Contains(menuCollection))
				stats.allMenus.Add(menuCollection);
			var menuIndex = stats.allMenus.IndexOf(menuCollection);
			...
			foreach (var element in menuCollection) {
				if (element is NesMiniApplication) {
					...
					Debug.WriteLine("Processing {0} ('{1}'), size: {2}KB", game.Code, game.Name, gameSize / 1024);
					...
					try {
						...
						ShowError(new Exception(string.Format(Resources.GameGenieFormatError, ex.Code, game.Name)), true);
					} catch (GameGenieNotFoundException ex) {
						ShowError(new Exception(string.Format(Resources.GameGenieNotFound, ex.Code, game.Name)), true);
					}
				}
		...
		private bool ExecuteTool(string tool, string args, out byte[] output, string directory = null,
			bool external = false) {
		...
		private static uint CalcKernelSize(byte[] header) {
			if (Encoding.ASCII.GetString(header, 0, 8) != "ANDROID!") throw new Exception(Resources.InvalidKernelHeader);
			var kernel_size = (uint) (header[8] | (header[9] * 0x100) | (header[10] * 0x10000) | (header[11] * 0x1000000));
			var kernel_addr = (uint) (header[12] | (header[13] * 0x100) | (header[14] * 0x10000) | (header[15] * 0x1000000));
			var ramdisk_size = (uint) (header[16] | (header[17] * 0x100) | (header[18] * 0x10000) | (header[19] * 0x1000000));
			var ramdisk_addr = (uint) (header[20] | (header[21] * 0x100) | (header[22] * 0x10000) | (header[23] * 0x1000000));
			var second_size = (uint) (header[24] | (header[25] * 0x100) | (header[26] * 0x10000) | (header[27] * 0x1000000));
			var second_addr = (uint) (header[28] | (header[29] * 0x100) | (header[30] * 0x10000) | (header[31] * 0x1000000));
			var tags_addr = (uint) (header[32] | (header[33] * 0x100) | (header[34] * 0x10000) | (header[35] * 0x1000000));
			var page_size = (uint) (header[36] | (header[37] * 0x100) | (header[38] * 0x10000) | (header[39] * 0x1000000));
			var dt_size = (uint) (header[40] | (header[41] * 0x100) | (header[42] * 0x10000) | (header[43] * 0x1000000));
			uint pages = 1;
		...
		public ICollection<NesMiniApplication> AddGames(IEnumerable<string> files, Form parentForm = null) {
			...
			foreach (var sourceFileName in files) {
				...
				try {
					...
					var needPatch = YesForAllPatches ? (bool?) true : null;
					byte[] rawData = null;
					string tmp = null;
					if (ext == ".7z" || ext == ".zip" || ext == ".rar") {
						SevenZipBase.SetLibraryPath(Path.Combine(baseDirectoryInternal,
						...
						using (var szExtractor = new SevenZipExtractor(sourceFileName)) {
							...
							if (fileName != sourceFileName) {
								...
								if (Path.GetExtension(fileName).ToLower() == ".desktop" // App in archive, need the whole directory
								    || szExtractor.ArchiveFileNames.Contains(Path.GetFileNameWithoutExtension(fileName) +
								                                             ".jpg") // Or it has cover in archive
					...
					var lGameGeniePath =
						Path.Combine(Path.GetDirectoryName(fileName), Path.GetFileNameWithoutExtension(fileName) + ".xml");
					if (File.Exists(lGameGeniePath)) {
						var lGameGenieDataBase = new GameGenieDataBase(app);
		...
		private void DownloadCovers() {
			...
			foreach (NesMiniApplication game in Games) {
				...
				for (var tries = 0; urls != null && tries < 5 && tries < urls.Length; tries++)
					try {
		...
		private delegate DialogResult MessageBoxFromThreadDelegate(IWin32Window owner, string text, string caption,
			MessageBoxButtons buttons,
			MessageBoxIcon icon, MessageBoxDefaultButton defaultButton, bool tweak);

		private class GamesTreeStats {
			public readonly List<NesMenuCollection> allMenus = new List<NesMenuCollection>();
