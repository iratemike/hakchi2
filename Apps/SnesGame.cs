...
namespace com.clusterrr.hakchi_gui {
	public class SnesGame : NesMiniApplication {
		...
		private const string DefaultArgs = "--volume 100 -rollback-snapshot-period 600";
		private static readonly List<byte> SfxTypes = new List<byte> {0x13, 0x14, 0x15, 0x1a};
		private static readonly List<byte> Dsp1Types = new List<byte> {0x03, 0x05};
		private static readonly List<byte> SA1Types = new List<byte> {0x34, 0x35};

		private static readonly Dictionary<string, ushort> knownPresets = new Dictionary<string, ushort> {
		...
		public override string GoogleSuffix => "(snes | super nintendo)";
		...
		private static void MakeSfrom(ref byte[] rawRomData) {
			...
			if (romHeaderLoRom.GameTitle.Length == 0)
				loRom = false;
			foreach (var c in romHeaderLoRom.GameTitle)
				if (c < 31 || c > 127) loRom = false;
			if (romHeaderHiRom.GameTitle.Length == 0)
				hiRom = false;
			foreach (var c in romHeaderHiRom.GameTitle)
			...
			var gameTitle = romHeader.GameTitle.Trim();
		...
		private struct SnesRomHeader {
			[MarshalAs(UnmanagedType.ByValTStr, SizeConst = 21)] public readonly string GameTitle;

			[MarshalAs(UnmanagedType.U1)] // $xFD5
			public readonly byte RomMakeup;

			[MarshalAs(UnmanagedType.U1)] // $xFD6
			public readonly byte RomType;

			[MarshalAs(UnmanagedType.U1)] // $xFD7
			public readonly byte RomSize;

			[MarshalAs(UnmanagedType.U1)] // $xFD8
			public readonly byte SramSize;

			[MarshalAs(UnmanagedType.U2)] // $xFD9
			public readonly ushort LicenseId;

			[MarshalAs(UnmanagedType.U1)] // $xFDB
			public readonly byte Version;

			[MarshalAs(UnmanagedType.U2)] // $xFDC
			public readonly ushort ChecksumComplement;
		...
		public struct SfromHeader1 {
			[MarshalAs(UnmanagedType.U4)] public uint Uknown1_0x00000100;
			[MarshalAs(UnmanagedType.U4)] public uint FileSize;
			[MarshalAs(UnmanagedType.U4)] public uint Uknown2_0x00000030;
			[MarshalAs(UnmanagedType.U4)] public uint RomEnd;
			[MarshalAs(UnmanagedType.U4)] public uint FooterStart;
			[MarshalAs(UnmanagedType.U4)] public uint Header2;
			[MarshalAs(UnmanagedType.U4)] public uint FileSize2;
			[MarshalAs(UnmanagedType.U4)] public uint Uknown3_0x00000000;
			[MarshalAs(UnmanagedType.U4)] public uint Flags;
			[MarshalAs(UnmanagedType.ByValArray, SizeConst = 8)] public byte[] VCGameID;
			[MarshalAs(UnmanagedType.U4)] public uint Uknown4_0x00000000;

			public SfromHeader1(uint romSize) {
				Uknown1_0x00000100 = 0x00000100;
				FileSize = 48 + romSize + 38;
		...
		public struct SfromHeader2 {
			...
			[MarshalAs(UnmanagedType.U4)] public uint Unknown4_0x00000000;
